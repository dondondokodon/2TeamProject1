...
void ProjectileHoming::Update(float elapsedTime)
{
	...
	//旋回
	{
		...
		if (lengthSq > 0.00001f)
		{
			...
			//前方向ベクトルとターゲットへのまでのベクトルの内積（角度）を算出
			dot = DirectX::XMVectorGetX(DirectX::XMVector3Dot(Direction, Vec));
			...
			if (fabsf(rot)>0.0001f)
			{
				...
				//回転軸と回転量から回転行列を算出
				DirectX::XMMATRIX Rotation = DirectX::XMMatrixRotationAxis(Axis, rot);	//ここの２爪の引数にturnspeed使ってたから上でやったrotの意味がなかったのでおかしかった

				//現在の行列を回転させる
				DirectX::XMMATRIX Transform = DirectX::XMLoadFloat4x4(&transform);
				Transform = DirectX::XMMatrixMultiply(Transform, Rotation);				//この関数あんまりわからん

				//回転後の前方方向を取り出し、単位ベクトル化する
				Direction = DirectX::XMVector3Normalize(Transform.r[2]);				//ここもdirectionにしてたから回転まえの前方向を参照していたのでうまくいってなかった
