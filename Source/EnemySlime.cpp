...
EnemySlime::EnemySlime()
{
	...
	hitEffect = new Effect("Data/Effect/bakuhatu.efkefc");
...
void EnemySlime::SetRandomTargetPosition()
{
	...
	targetPosition.z = territoryOrigin.z + cos(theta) * range;
}
...
void EnemySlime::MoveToTarget(float elapsedTime, float moveSpeedRate, float turnSpeedRate)
{
	...
	Turn(elapsedTime, vx, vz, moveSpeed * turnSpeedRate);
}
...
void EnemySlime::CollisionProjectilesVsPlayer()
{
	...
	for (int i = 0;i < projectileCount;i++)
	{
		...
		{
			//ダメージを与える
			if (Player::Instance().ApplyDamage(1, 0.5f))	//ノーダメージ
			{
				//吹き飛ばす
				{
					...
					DirectX::XMVECTOR dir = DirectX::XMVectorSubtract(playerPos, pPos);//向き
					...
					Player::Instance().AddImpulse(impulse);
				}

				//ヒットエフェクト再生
				{
					DirectX::XMFLOAT3 p = Player::Instance().GetPosition();
					p.y += Player::Instance().GetHeight() * 0.5f;
					hitEffect->Play(p);
				}
