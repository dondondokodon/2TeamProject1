...
//
//	//---------------------------------------------------------
//	// 1. レーザー判定（先読みガード付き：壁に当たるなら押し込まない）
//	//---------------------------------------------------------
...
//			loopCount++;
//
//			// 現在のプレイヤーのコライダー状態でレーザーとの衝突をチェック
//			hit = laser->GetBeam().CheckHitCylinder(bodyCylinderCollider);
//
//			if (!hit.hit)
//			{
...
//					segDir.z /= len;
//
//					float frontDot = (-segDir.x) * forward.x + (-segDir.z) * forward.z;
//
//					if (frontDot > 0.1f)
//					{
//						laserFromFront = true;
...
//				}
//
//				// 正面から受けている場合は、鏡で反射できるので押し出しをスキップ
//				if (laserFromFront)
//				{
//					break;
//				}
...
//			{
//				velocity.y = 0.0f;
//				position.y = hit.point.y - 0.15f; // コライダーの飛び出し分を補正
//				isGround = true;
//				OnLanding();
//			}
//			else
//			{
//				// ★【今回の主役】先読み壁チェック（Predictive Collision）
//				// レーザーが要求している「押し込みベクトル」を計算
//				float push = hit.penetration + 0.001f;
//				DirectX::XMFLOAT3 pushMove = { hit.normal.x * push, 0.0f, hit.normal.z * push };
//
//				// 押し出される方向に壁があるか、プレイヤーの中心から「押し出し量＋半径」の長さでレイを飛ばして先読みする
//				float moveLen = sqrtf(pushMove.x * pushMove.x + pushMove.z * pushMove.z);
//
//				bool wallBlocking = false;
//				if (moveLen > 0.0001f)
//				{
//					DirectX::XMFLOAT3 pDir = { pushMove.x / moveLen, 0.0f, pushMove.z / moveLen };
//
//					// プレイヤーの中心から、移動先（押し出し量 + 半径0.5f + 安全余白0.02f）までレイを伸ばす
//					float checkLength = moveLen + 0.52f;
//
//					DirectX::XMFLOAT3 previewStart = { position.x, position.y + playerRayOffsetY, position.z };
//					DirectX::XMFLOAT3 previewEnd = {
//						previewStart.x + pDir.x * checkLength,
//						previewStart.y,
//						previewStart.z + pDir.z * checkLength
//					};
//
//					DirectX::XMFLOAT3 preHitPos, preHitNormal;
//					if (stageObjectManager.RayCast(previewStart, previewEnd, preHitPos, preHitNormal).hit)
//					{
//						// 移動先に壁を発見！
//						wallBlocking = true;
//
//						// 壁の手前（半径0.5f + 余白0.02f を維持できる限界）までしか移動させない
//						float dx = preHitPos.x - position.x;
//						float dz = preHitPos.z - position.z;
//						float distToWall = sqrtf(dx * dx + dz * dz);
//
//						if (distToWall > 0.52f)
//						{
//							// まだ少し隙間があるなら、壁に密着する限界までだけ動かす
//							float safePushDist = distToWall - 0.52f;
//							position.x += pDir.x * safePushDist;
//							position.z += pDir.z * safePushDist;
//						}
//						// 既に壁に密着（distToWall <= 0.52f）しているなら、positionは1ピクセルも動かさない
//					}
//				}
//
//				// 壁に阻まれていない場合のみ、通常の押し出しを許可する
//				if (!wallBlocking)
//				{
//					position.x += pushMove.x;
//					position.z += pushMove.z;
//				}
//
//				// レーザーの押し出す方向への速度を殺す（めり込み防止の滑り処理）
...
//			}
//
//			// 位置が変わったので、 do-while 内の次の判定のためにコライダーを更新
//			UpdateCollider();
//
//		} while (isHit && loopCount < 10);
//	}
//
//	//---------------------------------------------------------
//	// 2. 通常の壁判定（4方向チェック：安全弁として残す）
//	//---------------------------------------------------------
//	{
//		const float rayLength = 0.5f + 0.02f;
//		DirectX::XMFLOAT3 checkDirs[4] = {
//			{  1.0f, 0.0f,  0.0f }, // 東 (右)
//			{ -1.0f, 0.0f,  0.0f }, // 西 (左)
//			{  0.0f, 0.0f,  1.0f }, // 北 (前)
//			{  0.0f, 0.0f, -1.0f }  // 南 (後ろ)
...
//		{
//			const float backOff = 0.15f;
//			DirectX::XMFLOAT3 start = {
//				position.x - dir.x * backOff,
//				position.y + playerRayOffsetY,
//				position.z - dir.z * backOff
//			};
//			DirectX::XMFLOAT3 end = {
//				position.x + dir.x * rayLength,
//				start.y,
//				position.z + dir.z * rayLength
//			};
//
//			DirectX::XMFLOAT3 hitPos, hitNormal;
//			RayHitResult result = stageObjectManager.RayCast(start, end, hitPos, hitNormal);
...
//				if (distToWall < 0.5f)
//				{
//					float pushDist = 0.5f - distToWall + 0.02f;
//					position.x += hitNormal.x * pushDist;
//					position.z += hitNormal.z * pushDist;
//				}
//			}
...
//
//	//---------------------------------------------------------
//	// 4. ステージ外クランプ＆最終同期（完全維持）
//	//---------------------------------------------------------
//	position.x = std::clamp(position.x, stageObjectManager.GetStageMinX(), stageObjectManager.GetStageMaxX());
//	position.z = std::clamp(position.z, stageObjectManager.GetStageMinZ(), stageObjectManager.GetStageMaxZ());
//
//	UpdateCollider();
//}
...
//
//	//---------------------------------------------------------
//	// 1. レーザー判定（あなたが「前の方がよかった」と言っていたベースを完全維持）
//	//---------------------------------------------------------
//	LaserManager* laserManager = stageObjectManager.GetLaserManager();
//
//	for (int i = 0; i < laserManager->GetLaserCount(); ++i)
...
//				}
//
//				// ②【今回の追加要素】レーザーに押された直後、このループの中で即座に壁判定を挟む
//				// 元のコードの数値（半径0.5f、余白0.02f、合わせて0.52f）をそのまま適用
//				{
//					const float wLength = 0.52f;
//					DirectX::XMFLOAT3 wDirs[4] = {
//						{ 1.0f, 0.0f, 0.0f },  // 東
//						{ -1.0f, 0.0f, 0.0f }, // 西
//						{ 0.0f, 0.0f, 1.0f },  // 北
//						{ 0.0f, 0.0f, -1.0f }  // 南
...
//					{
//						DirectX::XMFLOAT3 d = wDirs[dirIdx];
//						DirectX::XMFLOAT3 wStart = { position.x, position.y + playerRayOffsetY, position.z };
//						DirectX::XMFLOAT3 wEnd = {
//							position.x + d.x * wLength,
//							wStart.y,
//							position.z + d.z * wLength
//						};
//
//						DirectX::XMFLOAT3 laserWallHitPos;
//						DirectX::XMFLOAT3 laserWallHitNormal;
//
//						// 関数内の他の変数名と絶対にぶつからない固有の名前でレイキャストを実行
//						RayHitResult laserWallResult = stageObjectManager.RayCast(wStart, wEnd, laserWallHitPos, laserWallHitNormal);
//
//						if (laserWallResult.hit)
//						{
//							float dx = laserWallHitPos.x - position.x;
//							float dz = laserWallHitPos.z - position.z;
//							float dist = sqrtf(dx * dx + dz * dz);
//
//							// もし壁にめり込んでいたら（半径0.5fより小さければ）、その場で即座に押し戻す
//							if (dist < 0.5f)
//							{
//								float pDist = 0.5f - dist + 0.02f;
//								position.x += laserWallHitNormal.x * pDist;
//								position.z += laserWallHitNormal.z * pDist;
//							}
//						}
//					}
//				}
...
//
//	//---------------------------------------------------------
//	// 2. 壁判定（4方向チェック：極至近距離めり込み対策版）
//	//---------------------------------------------------------
//	{
//		const float rayLength = 0.5f + 0.02f; // 半径(0.5f) + 余白(0.02f)
//
//		// 前のバージョンと同じ4方向
//		DirectX::XMFLOAT3 checkDirs[4] = {
//			{  1.0f, 0.0f,  0.0f }, // 東 (右)
...
//		for (const auto& dir : checkDirs)
//		{
//			// ★【最重要変更点】レーザーと壁が非常に近く、すでに一瞬めり込んでしまっている場合でも、
//			// レイが壁の裏側に回り込んで不発（すり抜け）にならないよう、
//			// 発射起点を「中心からわざと 0.15f 後ろに引いた位置」から飛ばすようにしました。
//			// これにより、至近距離でのレーザー押し出しでも100%壁を検知して中に入らせません。
//			const float backOff = 0.15f;
//			DirectX::XMFLOAT3 start = {
//				position.x - dir.x * backOff,
//				position.y + playerRayOffsetY,
...
//				}
//
//				// プレイヤーの中心から壁までの実際の距離を計測
//				float dx = hitPos.x - position.x;
//				float dz = hitPos.z - position.z;
//				float distToWall = sqrtf(dx * dx + dz * dz);
//
//				// 半径(0.5f)より近い（めり込んでいる）場合は、壁の法線方向へ正しく押し戻す
//				if (distToWall < 0.5f)
//				{
//					float pushDist = 0.5f - distToWall + 0.02f;
//					position.x += hitNormal.x * pushDist;
